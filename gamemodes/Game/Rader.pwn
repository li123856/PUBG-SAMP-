...
stock GetPlayer2DRadarZone(playerid)
{
	new line[MAX_ZONE_NAME];
	format(line,sizeof(line)," ");
	forex(i,sizeof(gSAZones))
 	{
		if(PlayerPos[playerid][_X] >= gSAZones[i][SAZONE_AREA][0] && PlayerPos[playerid][_X] <= gSAZones[i][SAZONE_AREA][3] && PlayerPos[playerid][_Y] >= gSAZones[i][SAZONE_AREA][1] && PlayerPos[playerid][_Y] <= gSAZones[i][SAZONE_AREA][4])
		{
		    format(line,sizeof(line),gSAZones[i][SAZONE_NAME]);
		    return line;
		}
	}
	return line;
}
N::CreatePlayerRaderDraw(playerid)
{
    DestoryPlayerRaderDraw(playerid);
	RaderZoneDraw[playerid] = CreatePlayerTextDraw(playerid,635.000000, 435.000000, " ");
	PlayerTextDrawAlignment(playerid,RaderZoneDraw[playerid], 3);
	PlayerTextDrawBackgroundColor(playerid,RaderZoneDraw[playerid], 255);
	PlayerTextDrawFont(playerid,RaderZoneDraw[playerid], 2);
	PlayerTextDrawLetterSize(playerid,RaderZoneDraw[playerid],0.280000, 1.100000);
	PlayerTextDrawColor(playerid,RaderZoneDraw[playerid], -1);
	PlayerTextDrawSetOutline(playerid,RaderZoneDraw[playerid], 1);
	PlayerTextDrawSetProportional(playerid,RaderZoneDraw[playerid], 1);
	return 1;
}
N::UpdatePlayerRaderDraw(playerid)
{
	if(RaderZoneDraw[playerid]!=PlayerText:INVALID_TEXT_DRAW)
	{
	    /*new Line[MAX_ZONE_NAME];
	    format(Line,sizeof(Line),"%s",Line);*/
	    PlayerTextDrawSetString(playerid,RaderZoneDraw[playerid],GetPlayer2DRadarZone(playerid));
	    PlayerTextDrawShow(playerid,RaderZoneDraw[playerid]);
    }
	return 1;
}
N::DestoryPlayerRaderDraw(playerid)
{
	if(RaderZoneDraw[playerid]!=PlayerText:INVALID_TEXT_DRAW)
	{
		PlayerTextDrawDestroy(playerid,RaderZoneDraw[playerid]);
		RaderZoneDraw[playerid]=PlayerText:INVALID_TEXT_DRAW;
	}
	return 1;
}
